    ...
    public class HediffComp_GiveHediffsInRangeUndead : HediffComp
    {
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            foreach (Pawn pawn in readOnlyList)
            {
                ...
                if ((pawn.RaceProps.Humanlike && pawn.genes.HasActiveGene(Props.geneDef) || (pawn.RaceProps.IsMechanoid && (extension == true))) && !pawn.Dead && pawn.health != null && pawn != this.parent.pawn && pawn.Position.DistanceTo(this.parent.pawn.Position) <= this.Props.range && this.Props.targetingParameters.CanTarget(pawn, null))
                {
